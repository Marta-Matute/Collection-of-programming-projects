...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    // We do not parallelize this portion, since it makes the execution slower
    for( i=0; i<rows; i++ ){
        for( j=0; j<columns; j++ ) {
    ...
    for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
        ...
        /* 4.1. Activate focal points */ //probably not worth the paralelization
        ...
        #pragma acc data copy(surface[:mat_size]) create(surfaceCopy[mat_size]) copyin(focal[:num_focal])
        {
            for( step=0; step<10; step++ )	{
                ...
                #pragma acc parallel vector_length((columns))
		{
	...
	if (num_deactivated == num_focal) {
	    ...
	    #pragma acc parallel
	    {
	    #pragma acc loop collapse(2) reduction(max: global_residual) private(j)
            for( i=1; i<rows-1; i++ ){
		for( j=1; j<columns-1; j++ ){
                    global_residual = global_residual > fabs( accessMat( surface, i, j ) - accessMat( surfaceCopy, i, j ) )
                    ?
	...
	# pragma acc parallel loop gang vector vector_length(num_teams)
	    ...
	    #pragma acc loop seq
            ...
            float ftx = focal[target].x, fty = focal[target].y;
            float tx = teams[t].x, ty = teams[t].y;
            if ( teams[t].type == 1 ) {
                ...
                if ( fty < ty ) teams[t].y--;
                else if ( fty > ty ) teams[t].y++;
                else if ( ftx < tx ) teams[t].x--;
                else if ( ftx > tx ) teams[t].x++;
            }
            else {
